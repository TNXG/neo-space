...
export const ContentLayout = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () => (
      <div
        ref={layoutRef}
        class="bg-[var(--content-bg)] @container flex flex-col h-full"
      >
          ...
          >
            <div class="flex gap-3 min-w-0 items-center">
              ...
              {(isMobile.value || ui.sidebarCollapse.value) && (
                <button
                  class="text-[var(--sidebar-text)] rounded-lg flex shrink-0 h-8 w-8 transition-colors items-center justify-center hover:text-[var(--sidebar-text-active)] hover:bg-[var(--sidebar-hover-bg)]"
              ...
              <h1 class="text-lg text-[var(--sidebar-text-active)] font-semibold shrink-0">
                {pageTitle.value}
              </h1>
              {/* Header 副标题/元信息 */}
              {headerSubtitle.value && (
                <div class="ml-2 pl-3 border-l border-neutral-200 min-w-0 truncate dark:border-neutral-700">
                  {headerSubtitle.value}
                </div>
              )}
            </div>
