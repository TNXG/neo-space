  ...
  setup() {
    ...
    const handleLoad = async () => {
      ...
      try {
        ...
        result.value = data
        if (!data) toast.info('暂无缓存的时光胶囊记录')
      } catch {
        toast.error('读取失败')
      } finally {
        loading.value = false
        ...
        <NCard title="结果">
          {loading.value ? (
            ...
            <NSpace vertical size={12}>
              ...
              <div>
                <div class="mb-1 text-sm opacity-70">说明</div>
                <p class="whitespace-pre-wrap text-sm">{result.value.reason}</p>
              </div>
              {result.value.markers.length > 0 && (
                <div>
                  <div class="mb-1 text-sm opacity-70">关键词</div>
                  <NSpace size={4}>
