      ...
      >
        <NDrawerContent title="函数调用日志" closable>
          <div class="flex flex-col gap-3 h-full">
            {/* Status filter */}
            <div class="p-1 rounded-lg bg-neutral-100 flex gap-1 dark:bg-neutral-800">
              {statusFilters.map(f => (
                <button
                  key={f.value}
                  class={[
            ...
            {pagination.value && pagination.value.totalPage > 1 && (
              <div class="pt-3 border-t border-neutral-200 flex shrink-0 justify-center dark:border-neutral-800">
                <NPagination
                  page={page.value}
...
const LogItem = defineComponent({
  ...
  setup(props) {
    return () => (
      <div class="border border-neutral-200 rounded-lg overflow-hidden dark:border-neutral-800">
        <button
          class="px-3 py-2.5 text-left flex gap-3 w-full transition-colors items-center hover:bg-neutral-50 dark:hover:bg-neutral-800/50"
          onClick={props.onToggle}
        >
          {/* Expand icon */}
          {props.expanded
            ? (
                <ChevronDownIcon class="text-neutral-400 shrink-0 size-3.5" />
              )
            : (
                <ChevronRightIcon class="text-neutral-400 shrink-0 size-3.5" />
              )}
          ...
          {/* Execution time */}
          <span class="text-xs text-neutral-500 shrink-0 dark:text-neutral-400">
            {props.log.executionTime}
            ms
          </span>
          ...
          {/* IP */}
          <span class="text-xs text-neutral-400 shrink-0 dark:text-neutral-500">
            {props.log.ip}
          </span>
...
const LogDetail = defineComponent({
  ...
  setup(props) {
    ...
    return () => (
      <div class="px-3 py-3 border-t border-neutral-200 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900/50">
        {isLoading.value ? (
          <div class="py-4 flex items-center justify-center">
            <div class="border-2 border-neutral-300 border-t-neutral-900 rounded-full size-4 animate-spin dark:border-neutral-700 dark:border-t-white" />
          </div>
        ) : !detail.value ? (
          ...
          <div class="space-y-3">
            ...
            {detail.value.logs && detail.value.logs.length > 0 && (
              <div>
                <h4 class="text-xs text-neutral-500 font-medium mb-1.5 dark:text-neutral-400">
                  Console
                </h4>
                <div class="text-xs text-neutral-800 font-mono p-3 rounded-lg bg-neutral-100 dark:text-neutral-200 dark:bg-neutral-950">
                  {detail.value.logs.map((entry, i) => (
                    ...
                    >
                      <span class="text-neutral-500 mr-2 select-none dark:text-neutral-400">
                        [
                        {entry.level}
                        ]
                      </span>
            ...
            {detail.value.error && (
              <div>
                <h4 class="text-xs text-red-600 font-medium mb-1.5 dark:text-red-400">
                  Error
                </h4>
                <div class="text-xs text-red-900 font-mono p-3 rounded-lg bg-red-100 dark:text-red-100 dark:bg-red-950/90">
                  <div class="font-semibold">
                    {detail.value.error.name}
                    :
