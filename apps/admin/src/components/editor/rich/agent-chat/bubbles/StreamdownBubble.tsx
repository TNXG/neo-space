...
export const StreamdownBubble = defineComponent({
  ...
  setup(props) {
    ...
    return () => (
      <div
        ref={containerRef}
        class="text-sm leading-[1.75] [&_code]:text-xs [&_pre]:text-xs [&_a]:text-blue-600 [&_li]:my-0.5 [&_ol]:my-1.5 [&_p]:my-1.5 [&_pre]:my-2 [&_ul]:my-1.5 [&_pre_code]:p-0 [&_pre]:p-3 [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_pre]:rounded-lg [&_code]:bg-neutral-100 [&_pre_code]:bg-transparent [&_pre]:bg-neutral-50 [&_pre]:overflow-x-auto dark:[&_a]:text-blue-400 dark:[&_code]:bg-neutral-800 dark:[&_pre]:bg-neutral-900"
        v-html={
          html.value
          + (props.isStreaming
