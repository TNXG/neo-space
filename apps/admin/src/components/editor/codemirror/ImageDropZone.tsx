...
export const ImageDropZone = defineComponent({
  ...
  setup() {
    ...
    onBeforeUnmount(() => {
      if (boundView)
        unbindEvents(boundView);
    });

    return () => {
      if (!isDragging.value || !teleportTarget.value)
        return null;

      return (
        <Teleport to={teleportTarget.value}>
          <div class="bg-white/60 flex pointer-events-none items-center inset-0 justify-center absolute z-100 backdrop-blur-md dark:bg-neutral-950/70">
            <div class="animate-in zoom-in-95 px-16 py-12 border border-neutral-200 rounded-2xl bg-white flex flex-col gap-6 pointer-events-none shadow-2xl duration-150 items-center dark:border-neutral-700 dark:bg-neutral-800">
              <Upload
                size={48}
                strokeWidth={1.5}
                class="text-neutral-400 dark:text-neutral-500"
              />
              <span class="text-lg text-neutral-600 font-medium dark:text-neutral-300">
                松开以上传图片
              </span>
            </div>
          </div>
