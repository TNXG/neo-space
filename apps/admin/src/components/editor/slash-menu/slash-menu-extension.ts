...
export const slashMenuStateField = StateField.define<SlashMenuState>({
  ...
  update(value, tr) {
    ...
    if (!next.active) {
      ...
      if (selection.head !== slashPos + 1)
        return next;

      const doc = tr.newDoc;
      if (doc.sliceString(slashPos, slashPos + 1) !== "/")
        return next;
      ...
      if (!isLineStart && !isWhitespaceBefore)
        return next;
      if (isLineInBlock(tr.state, line.number))
        return next;
    ...
    if (triggerLine.number !== cursorLine.number)
      return shouldClose();
