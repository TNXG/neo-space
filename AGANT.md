...
- 本站后端 API 的实体主键字段以 MongoDB 默认的 `_id` 为准。
- 如果 admin 前端遇到 `id` / `_id` 不一致的问题，优先修改前端模型、adapter、调用参数和组件使用方式，不要为了适配 admin 去改后端返回字段。
- 前端内部可以按组件需要派生本地字段，但请求后端 API 时应使用 `_id`。
