...
export function CodeBlock({ children, className, language = "text", filename, style, fallbackText }: CodeBlockProps) {
	...
	return (
		<div className="group relative my-5 md:my-6 overflow-hidden rounded-xl border border-zinc-200 bg-white/50 shadow-sm transition-all hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950/50 backdrop-blur-sm">
			<div className="flex h-10 md:h-11 items-center justify-between border-b border-zinc-200/60 bg-zinc-50/50 px-3 md:px-4 dark:border-zinc-800/60 dark:bg-zinc-900/50">
				<div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1 overflow-hidden">
					...
					<div className="flex items-center gap-1.5 md:gap-2 select-none min-w-0 overflow-hidden">
						...
						<span className="text-[11px] md:text-xs font-medium text-zinc-600 dark:text-zinc-400 truncate">
							{filename || language}
						</span>
					</div>
				</div>
