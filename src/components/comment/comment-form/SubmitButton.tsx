...
export function SubmitButton({ onClick, disabled, isPending }: SubmitButtonProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3 py-2 sm:py-1.5 rounded-lg text-xs sm:text-xs font-bold transition-all shadow-sm min-h-9 sm:h-8 cursor-pointer",
        !disabled
