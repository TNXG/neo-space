...
import { useCommentRefresh } from "./CommentContext";
	...
	return (
		<div className="relative mt-4 w-full">
			<fieldset
				ref={containerRef}
				className={cn(
					"group relative flex flex-col gap-0 border-none p-0 m-0",
				)}
			>
				...
				{/* 编辑区 */}
				<div className="flex flex-col min-h-20 py-3 px-4 relative z-0">
					{preview
						? (
								<div className="min-h-[60px] prose prose-sm prose-stone dark:prose-invert max-w-none animate-fade-in">
						...
						: (
								<textarea
									...
									className="grow w-full bg-transparent text-base text-foreground placeholder:text-muted-foreground/60 outline-none resize-y min-h-[60px] max-h-[400px] disabled:opacity-50"
									spellCheck={false}
									disabled={isPending}
								/>
							)}
				...
				>
					<div className="flex flex-wrap items-center gap-3 px-4 pb-3 relative z-10">
						...
						<div className="flex items-center gap-4">
								...
								<AnimatePresence>
									{showEmoji && owoData && (
										<motion.div
											...
											className="absolute bottom-full left-0 mb-3 w-[300px] bg-background/95 backdrop-blur-xl border border-border/60 rounded-lg shadow-glass z-50 p-2"
										>
							...
							<div className="flex items-center gap-2">
								...
								/>
								<span className="text-xs text-muted-foreground select-none">预览</span>
							</div>
						</div>
										...
										>
											<div className="relative">
												<button
													...
													className={cn(
														"flex items-center gap-2 pl-1 pr-3 py-1 rounded-full border transition-all cursor-pointer",
												...
												>
													<img src={authUser.image} alt={authUser.name} className="size-6 rounded-full" />
													<span className="text-xs font-medium text-foreground max-w-[80px] truncate">{authUser.name}</span>
												</button>
										...
										>
											...
											<div className="flex items-center gap-2">
												<input
													...
													placeholder="昵称*"
													className="w-20 sm:w-24 bg-transparent border-b border-border/50 focus:border-accent-500 rounded-none px-1 py-1 text-xs outline-none transition-all placeholder:text-muted-foreground/50 text-center"
												/>
												<input
													value={user.email}
													onChange={e => setUser({ ...user, email: e.target.value })}
													placeholder="邮箱"
													className="hidden sm:block w-32 bg-transparent border-b border-border/50 focus:border-accent-500 rounded-none px-1 py-1 text-xs outline-none transition-all placeholder:text-muted-foreground/50 text-center"
												/>
											</div>
											...
											<div className="relative">
												<button
													...
													className={cn(
														"p-1.5 rounded-full transition-all cursor-pointer", // 增加 cursor-pointer
														activePopover === "login"
													...
													title="登录"
												>
													<Icon icon="mingcute:user-3-line" className="w-4 h-4" />
												</button>
												...
												<AnimatePresence>
													{activePopover === "login" && (
														<motion.div
															...
															animate={{ opacity: 1, y: 0, scale: 1 }}
															exit={{ opacity: 0, y: 10, scale: 0.95 }}
															className="absolute bottom-full right-0 mb-3 z-50 w-[260px] bg-popover/95 backdrop-blur-xl border border-border/60 rounded-xl shadow-glass p-6 overflow-hidden"
														>
															<div className="text-center mb-5">
																<h3 className="font-serif text-xl font-medium text-foreground">登录</h3>
															</div>
															...
															{isRedirecting
																? (
																		...
																		>
																			<Icon icon="svg-spinners:ring-resize" className="size-8 text-accent-500" />
																			<span className="text-xs text-muted-foreground animate-pulse">正在前往登录...</span>
																		</motion.div>
																	)
																: (
																		<motion.div
																			...
																			animate={{ opacity: 1 }}
																		>
																			<ul className="space-y-3 mb-5 text-xs text-muted-foreground">
																				<li className="flex items-center gap-3">
																					<Icon icon="mingcute:check-circle-line" className="text-accent-500 shrink-0" />
																					<span>无需进行人机验证</span>
																				</li>
																				<li className="flex items-center gap-3">
																					<Icon icon="mingcute:check-circle-line" className="text-accent-500 shrink-0" />
																			...
																			*/}
																			...
																			>
																				<div className="
																	[&_button]:w-full [&_button]:flex [&_button]:items-center [&_button]:justify-center [&_button]:gap-2 [&_button]:py-2 [&_button]:rounded-md
																	[&_button]:border [&_button]:border-border [&_button]:bg-background
																	[&_button]:text-xs [&_button]:font-medium [&_button]:text-foreground
																...
																"
																			...
																			<button
																				...
																				className="w-full mt-4 py-1 text-xs text-muted-foreground/60 hover:text-foreground transition-colors bg-muted/30 rounded cursor-pointer"
																			>
																				取消
																			</button>
																		</motion.div>
...
function SubmitButton({ onClick, disabled, isPending }: { onClick: () => void; disabled: boolean; isPending: boolean }) {
	return (
		<button
			...
			disabled={disabled}
			className={cn(
				"flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm h-8 cursor-pointer", // cursor-pointer
				!disabled
					? "bg-accent-600 text-white hover:bg-accent-500 hover:shadow-accent-500/20 active:scale-95"
					: "bg-muted text-muted-foreground cursor-not-allowed opacity-50",
			)}
		>
			{isPending ? <Icon icon="svg-spinners:ring-resize" /> : <Icon icon="mingcute:send-plane-fill" />}
