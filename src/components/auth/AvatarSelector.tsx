...
export function AvatarSelector({ accounts, currentAvatar, userEmail, onAvatarChange }: AvatarSelectorProps) {
	...
	return (
		<div className="space-y-2">
			<label className="text-sm font-medium text-foreground">头像来源</label>
			...
			<div className="relative">
				<select
					...
					disabled={isUpdating}
					className="w-full appearance-none rounded-lg border border-border bg-background px-3 py-2 pr-10 text-sm text-foreground outline-none transition-colors hover:border-accent-300 focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20 disabled:cursor-not-allowed disabled:opacity-50"
				>
					{options.map(option => (
				...
				<div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2">
					{isUpdating
						? (
								<Icon icon="svg-spinners:ring-resize" className="size-4 text-muted-foreground" />
							)
						: (
								<Icon icon="mingcute:down-line" className="size-4 text-muted-foreground" />
							)}
				</div>
			</div>
