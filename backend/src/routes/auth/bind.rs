...
pub async fn bind_anonymous_identity(
    ...
    let anon_reader = reader_repo
        .find_by_name_and_email(&request.name, &request.email)
        .await
    ...
    // 迁移所有 Account
    id_service
        .merge_identities(auth.user_id, anon_reader.id)
        .await
    ...
    let token = id_service
        .issue_token(anon_reader.id, anon_reader.is_owner)
        .map_err(ApiResponse::internal_error)?;

    Ok(ApiResponse::json_success_with_message(
        anon_reader.into(),
        token,
    ))
}
...
pub async fn skip_bind(
    ...
    let accounts = account_repo
        ...
        .await
        .map_err(|_| ApiResponse::internal_error("查询失败".to_string()))?;
    let acc = accounts
        .first()
        .ok_or_else(|| ApiResponse::not_found("未找到账号信息".to_string()))?;
    ...
    let new_reader = Reader {
        ...
        handle: acc
            .oauth_handle
            .clone()
            .unwrap_or_else(|| Reader::generate_handle("user")),
    ...
    let new_id = reader_repo
        .create_reader(&new_reader)
        .await
        .map_err(|_| ApiResponse::internal_error("创建失败".to_string()))?;
    id_service
        .merge_identities(auth.user_id, new_id)
        .await
        .map_err(ApiResponse::internal_error)?;

    let token = id_service
        .issue_token(new_id, is_first)
        .map_err(ApiResponse::internal_error)?;
    Ok(ApiResponse::json_success_with_message(
        new_reader.into(),
        token,
...
pub async fn get_bindable_identities(
    ...
    let emails: Vec<String> = accounts
        .iter()
        .filter_map(|a| a.oauth_email.clone())
        .collect();

    let all_readers = reader_repo
        .get_all()
        .await
        .map_err(|_| ApiResponse::internal_error("数据库失败".to_string()))?;
    let bindable = all_readers
        .into_iter()
