...
### 10. UI/UX 与 自定义 UI/UX 设计系统 (核心)

**本项目采用全自定义的“极简通透+跨文化符号融合”设计系统。**

**10.1 CSS 变量与主题架构 (`globals.css`)**
必须手动定义一套完整的语义化 CSS 变量，以支持**自适应暗色模式**和全局风格统一。
*   **背景层：**
    *   `--bg-primary`: 页面主背景 (Light: `rgba(255, 255, 255, 0.95)`; Dark: `rgba(20, 22, 28, 1)`)
    *   `--bg-secondary`: 卡片等次级背景 (Light: `rgba(245, 245, 247, 1)`; Dark: `rgba(30, 32, 40, 0.95)`)
*   **玻璃层 (核心)：**
    *   `--bg-glass`: 叠加层的磨砂玻璃背景 (Light: `rgba(255, 255, 255, 0.6)`; Dark: `rgba(30, 32, 40, 0.6)`)
*   **内容层：**
    *   `--text-primary`: 主要文本 (确保与背景有 >4.5:1 的对比度)
    *   `--text-secondary`: 次要文本 (时间戳、描述等)
    *   `--accent`: 全局唯一的强调色 (如链接、Hover) (Light/Dark: `rgba(66, 153, 225, 0.7)`)
*   **实现：** 在 `tailwind.config.ts` 中扩展 `theme` 来消费这些 CSS 变量。所有组件的样式**必须**引用这些变量，**严禁**硬编码颜色值。

**10.2 布局与响应式策略 (Mobile First)**
*   **默认形态：** **单列垂直流布局**。所有内容优先以线性堆叠形式呈现，保证在任何窄屏设备上的可读性。
*   **响应式规则：**
    *   **严禁使用任何固定像素宽度** (如 `width: 600px`) 进行主布局。必须使用百分比、`vw/vh`、`min/max-width` 等流式单位。
    *   **弹性排版：** **必须**使用 CSS Grid (Auto-fill) 或 Flex (Wrap) 实现。
    *   **断点逻辑：** 仅在视口宽度超过预设阈值 (建议 ≥1200px) 时，布局才**自动**切换为双列或多列卡片网格。

**10.3 视觉美学与交互 (Visual Aesthetics & UX)**
*   **磨砂质感 (Glassmorphism)：**
    *   所有容器、卡片、导航栏、面板**必须**应用 **“弱模糊 + 半透明”** 效果 (`backdrop-filter: blur(8px)`)。
    *   通过 `background-color: var(--bg-glass)` 实现半透明背景。
*   **圆角规范：**
    *   所有卡片、容器、输入框等，**统一**采用 **16px - 18px** 的大圆角。
    *   小型交互元素（如标签、动作按钮）采用**胶囊形** (`border-radius: 100px`)。
*   **字体与排版：**
    *   **字体：** 统一使用无衬线字体（Inter 或 思源黑体）。
    *   **行高：** 正文行高设为 `1.6`，保证长文阅读舒适度。
    *   **留白：** 强制使用 `tailwindcss` 的间距尺度 (`space-y-4`, `gap-8`)，创造有呼吸感的布局。
*   **交互模式 (Z-axis Interaction)：**
    *   **严禁使用阻断式弹窗 (Blocking Modals)。** 所有次级界面（如文章详情、设置）**必须**通过以下非模态方式打开：
        *   横屏：**侧边抽屉 (Side Drawer)**
        *   竖屏：**底部上滑面板 (Bottom Sheet)**
    *   **动效规范：** 打开新视图时，主视图（背景层）执行 `transform: scale(0.98)` 并增加模糊度，创造 Z 轴纵深感；新视图平滑滑入。动画时长 `300ms`，缓动函数 `ease-in-out`。
*   **指针与反馈：**
    *   所有可交互元素（按钮、链接、可点击的 div）**必须**在 `hover` 时显示 `cursor-pointer`。
    *   禁用状态的元素**必须**显示 `cursor-not-allowed` 并降低透明度。
